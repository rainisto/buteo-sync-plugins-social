...
bool OneDriveImageSyncAdaptor::haveAlreadyCachedImage(const QString &imageId, const QString &imageUrl)
{
    OneDriveImage::ConstPtr dbImage = m_db.image(imageId);
    ...
    if (dbImageUrl != imageUrl) {
        SOCIALD_LOG_ERROR("Image/onedrive.db has outdated data!\n"
                          "   photoId:" << imageId << "\n"
                                                  ...
                                                  const QString &accessToken)
{
    ...
    // We need to add the user. We call OneDrive to get the informations that we
